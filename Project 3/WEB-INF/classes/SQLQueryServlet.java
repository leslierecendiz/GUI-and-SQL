...
public class SQLQueryServlet extends HttpServlet {
	...
	// Process the get request from the jsp front-end
	public void doGet(HttpServletRequest request, HttpServletResponse response)

			throws ServletException, IOException {
		String textBox = request.getParameter("textBox");//get the input
		String textBoxLowerCase = textBox.toLowerCase();//change it to lower case
		String result = null;
	...
	// Process the post request from the jsp front end
	...
	public String doSelectQuery(String textBox) throws SQLException {
		...
		String MakeColumnsHTML = "<thead class='thead-dark'><tr>";

		for (int i = 1; i <= numOfColumns; i++) {
			MakeColumnsHTML += "<th scope='col'>" + metaData.getColumnName(i) + "</th>";
		}

		MakeColumnsHTML += "</tr></thead>";
		...
		res = TableHTML + MakeColumnsHTML + TBodyHTML + TCloseHTML;
	...
	private String doUpdateQuery(String textBoxLowerCase) throws SQLException {
		...
		if (numOfShipmentsGreaterBefore < numOfShipmentsGreaterAfter) {
			...
			// Handle updates into shipments by using a left join with shipments and temp
			// table.
			int numberOfRowsAffectedAfter5 = statement.executeUpdate(
